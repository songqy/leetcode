...
var compareVersion = function (version1, version2) {
  const vList1 = version1.split('.');
  const vList2 = version2.split('.');
  const maxLen = Math.max(vList1.length, vList2.length);
  for (let i = 0; i < maxLen; ++i) {
    const v1 = vList1[i] ? Number(vList1[i]) : 0;
    const v2 = vList2[i] ? Number(vList2[i]) : 0;
    if (v1 > v2) return 1;
    if (v1 < v2) return -1;
  }
  return 0;
...
console.log(compareVersion('0.00.12.3331', '7.000.000.000'));
